...
sed -i "s/-- ghc-options:/ghc-options: -j$THREADS/g" ${CABAL_CONFIG}
set +u
if [[ "$WORKAROUND_GHC_MMAP_CRASH" == "yes" ]]; then
  sed -i "s/ghc-options:/ghc-options: +RTS -xm20000000 -RTS -with-rtsopts=-xm20000000/g" ${CABAL_CONFIG}
fi
set -u
sed -i "s/^[- ]*jobs:.*/jobs: $CABAL_JOBS/g" ${CABAL_CONFIG}
sed -i "/remote-repo-cache:.*/d" ${CABAL_CONFIG}
cat ${CABAL_CONFIG}
